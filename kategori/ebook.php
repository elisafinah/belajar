...
if ((isset($_POST['submit'])) AND ($_POST['judul'] ) AND ($_POST['pengarang'] ) AND ($_POST['kategori'] )) {
  $judul = $_POST['judul'];
  $pengarang = $_POST['pengarang'];
  $kategori = $_POST['kategori'];
  $sql = mysql_query("SELECT * FROM upload WHERE judul LIKE '%$judul%' AND pengarang LIKE '%$pengarang%' 
  AND kategori LIKE '%$kategori%' LIMIT 0, 50")or die(mysql_error());
   ...
   $i = 1;
		...
		<tr class="<?php
                        if ($i % 2 == 0) {
							...
							echo "<p><a href='lihat_semua.php?id=" . $res['id'] . "'>Lihat</a></p>";
